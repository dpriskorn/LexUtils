...
#     language_style: LanguageStyle = LanguageStyle.FORMAL
#     type_of_reference: ReferenceType = ReferenceType.WRITTEN
#     source: SupportedExampleSources = SupportedExampleSources.WIKISOURCE
#     document_title: str = ""
#
#     def process(self, json=None, lexemes: Lexemes = None):
#         try:
...
#             nlp.add_pipe("sentencizer")
#         elif (
#             self.language_code == WikimediaLanguageCode.FRENCH
...
#             if config.min_word_count < sentence_length < config.max_word_count:
...
#         url = (
#             f"https://{self.language_code.value}.wikisource.org/w/api.php?"
...
#                         logger.info(
#                             f"Trying to find QID using the truncated title: {truncated_title}"
#                         )
#                         url = (
#                             f"https://{self.language_code.value}.wikisource.org/w/api.php?"
#                             f"action=query&prop=pageprops&ppprop=wikibase_item&"
...
#         else:
#             raise ValueError(
#                 f"Got {response.status_code} from the Wikisource API, see {url}"
#             )
#         if self.document_qid is None:
